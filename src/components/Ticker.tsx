...
export const Ticker: React.FC = React.memo(() => {
  ...
  return (
    <div className="w-full bg-zinc-50 dark:bg-zinc-900 text-zinc-700 dark:text-zinc-300 dark:bg-zinc-950 border-b border-zinc-200 dark:border-zinc-800 h-7 flex items-center overflow-hidden select-none z-50 relative font-mono text-[11px] font-bold">
      {/* Live Badge */}
      <div className="hidden sm:flex items-center gap-1.5 px-3 h-full bg-white dark:bg-black text-emerald-400 dark:text-acid uppercase tracking-wider text-[10px] font-black z-10 shrink-0 border-r border-zinc-200 dark:border-zinc-800">
        ...
        <div className="animate-marquee-gpu items-center">
          ...
          {tickerSegments.map((seg, idx) => (
            <div key={`seg1-${idx}`} className="flex items-center gap-1.5 px-4 whitespace-nowrap">
              <span className="text-zinc-500 dark:text-zinc-400 uppercase text-[10px]">{seg.label}:</span>
