...
export function TrendingGrid({ onSelectToken }: TrendingGridProps) {
  ...
  const allTokens: any[] = rawTokens.map((t: any) => {
    ...
    return {
      ...
      dexId,
      badge: t.badge || (change >= 50 ? "Hot Gainer" : vol >= 500000 ? "Top Volume" : "Trending"),
  ...
  const filteredTokens = useMemo(() => {
    if (activeTab === "gainers") {
      return [...allTokens].sort((a, b) => b.priceChange24h - a.priceChange24h).filter((t) => t.priceChange24h > 0);
    }
    if (activeTab === "top_volume") {
      return [...allTokens].sort((a, b) => b.volume24h - a.volume24h);
    }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-1">
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1 bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-white/10 rounded-xl p-1 text-[11px]">
            ...
            >
              🔥 GAINERS ({allTokens.filter((t) => t.priceChange24h > 0).length})
            </button>
            <button
              type="button"
            ...
            >
              💎 $1M+ MCAP ({allTokens.filter((t) => t.mcap >= 1_000_000).length})
            </button>
          </div>
          ...
          <button
            ...
            title="Refresh DexScreener Data"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3.5">
        {filteredTokens.map((token) => {
          ...
          return (
            <div
              key={token.id}
              onClick={() => onSelectToken?.(token.ca, token.symbol, token.name, token.iconUrl)}
            ...
            >
              {/* ── Card Header: Logo, Name & Dex Badge ───────────────────── */}
              <div className="flex items-start justify-between gap-2">
                ...
                {/* Dex Platform Badge */}
                <span className="text-[9px] font-black px-2 py-0.5 rounded-md bg-amber-500/10 text-amber-400 border border-amber-500/30 uppercase shrink-0">
                  {token.dexId}
                </span>
              </div>

              {/* ── Metrics Grid: Price, MCAP, 24h Change ────────────────── */}
              <div className="grid grid-cols-3 gap-1.5 py-1.5 px-2 bg-zinc-50 dark:bg-zinc-900/60 rounded-xl border border-zinc-200 dark:border-white/5 text-center">
                <div>
                  <span className="text-[9px] text-zinc-400 font-bold block uppercase">Price</span>
                  <span className="text-xs font-black text-zinc-900 dark:text-zinc-100 truncate block font-mono">
                    {token.priceFormatted}
                  </span>
                </div>

                <div>
                  <span className="text-[9px] text-zinc-400 font-bold block uppercase">MCAP</span>
                  <span className="text-xs font-black text-amber-500 dark:text-amber-400 truncate block font-mono">
                    {token.mcapFormatted}
                  </span>
                </div>

                <div>
                  <span className="text-[9px] text-zinc-400 font-bold block uppercase">24H</span>
                  <span
                    className={`text-xs font-black truncate block font-mono flex items-center justify-center gap-0.5 ${
              ...
              {/* ── Liquidity & Volume Row ───────────────────────────────── */}
              <div className="flex items-center justify-between text-[10px] text-zinc-400 px-1 font-bold">
                <span className="flex items-center gap-1">
                  <Activity className="w-3 h-3 text-amber-400" />
                  Vol: <strong className="text-zinc-200">{token.volumeFormatted}</strong>
                </span>
                <span className="flex items-center gap-1">
                  <Coins className="w-3 h-3 text-emerald-400" />
                  Liq: <strong className="text-zinc-200">{token.liquidityFormatted}</strong>
                </span>
              </div>
