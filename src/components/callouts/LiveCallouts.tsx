    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          {data?.updatedAt && (
            <span className="flex items-center gap-1.5 text-[10px] font-mono text-zinc-500 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 px-2.5 py-1 rounded-lg">
              <Clock className="w-3 h-3 text-zinc-500 dark:text-zinc-400" />
              <span>{new Date(data.updatedAt).toLocaleTimeString()}</span>
              <span className="text-zinc-600 border-l border-zinc-200 dark:border-zinc-800 pl-1.5 ml-0.5 flex items-center gap-1.5">
                <span>auto 60s</span>
                <span className="text-amber-500/80 font-bold border-l border-zinc-200 dark:border-zinc-800 pl-1.5">6H ACTIVE WINDOW</span>
              </span>
            </span>
          )}
          {isValidating && (
        ...
        <div className="flex flex-wrap items-center gap-1.5">
              ...
              <button
                ...
                className={`text-[11px] font-mono px-2.5 py-1 rounded-full border transition-all cursor-pointer select-none ${
                  isSelected
                    ...
                    : "bg-zinc-50 dark:bg-zinc-900/60 border-zinc-200 dark:border-zinc-800 text-zinc-500 hover:border-zinc-700 hover:text-zinc-400"
                }`}
              >
                {label}
                {count > 0 && (
          ...
          {props.selectedCaller && (
            <button
              ...
              className="text-[10px] font-mono px-2 py-1 rounded-full bg-zinc-800 hover:bg-zinc-700 border border-zinc-300 dark:border-zinc-700 text-zinc-500 dark:text-zinc-400 hover:text-zinc-200 transition-colors flex items-center gap-1 cursor-pointer"
            >
              <X className="w-3 h-3" />
      ...
      {isLoading && rawCallouts.length === 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {[1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="p-4 rounded-2xl bg-zinc-50 dark:bg-zinc-900/60 border border-zinc-200 dark:border-zinc-800 animate-pulse h-52"
            />
          ))}
        </div>
      ) : props.selectedCaller && filteredCallouts.length === 0 ? (
        /* Caller specific empty state (e.g. alonalon) */
        <div className="py-14 flex flex-col items-center gap-3 text-center rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/30">
          <Radio className="w-7 h-7 text-zinc-600" />
          <p className="text-sm font-mono text-zinc-700 dark:text-zinc-300 font-semibold">
          ...
          <button
            type="button"
            onClick={() => props.onSelectCaller?.(null)}
            className="mt-2 px-3.5 py-1.5 rounded-xl bg-zinc-800 hover:bg-zinc-700 border border-zinc-300 dark:border-zinc-700 text-xs font-mono text-zinc-800 dark:text-zinc-200 transition-colors cursor-pointer"
          >
            Clear filter
          </button>
        </div>
        ...
        /* General empty state */
        <div className="py-16 flex flex-col items-center gap-3 text-center rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/30">
          <Radio className="w-7 h-7 text-zinc-700" />
          <p className="text-sm font-mono text-zinc-500 dark:text-zinc-400 font-semibold">
...
const CalloutCardItem: React.FC<CardProps> = ({
  ...
  return (
    <div className="p-4 rounded-2xl bg-zinc-50 dark:bg-zinc-900/80 border border-zinc-200 dark:border-zinc-800 hover:border-zinc-700 flex flex-col gap-3 transition-colors">
      {/* Caller row */}
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="w-7 h-7 rounded-full bg-zinc-800 border border-zinc-300 dark:border-zinc-700 flex items-center justify-center text-[10px] font-bold text-zinc-700 dark:text-zinc-300 uppercase shrink-0">
            {card.callerLabel.slice(0, 2)}
          </div>
          <span className="text-xs font-bold text-zinc-950 dark:text-white font-mono truncate">
            {card.callerLabel}
          </span>
          <button
      ...
      {/* Media */}
      {card.mediaUrl && (
        <div className="rounded-xl overflow-hidden border border-zinc-200 dark:border-zinc-800 max-h-48 bg-white dark:bg-zinc-950">
          <Image
            src={card.mediaUrl}
            alt="callout media"
            width={480}
      ...
      {/* Thesis */}
      {card.thesis && (
        <p className="text-xs text-zinc-700 dark:text-zinc-300 italic leading-relaxed line-clamp-3 font-sans">
          &ldquo;{card.thesis}&rdquo;
        </p>
      )}

      {/* Dynamic Target Token row: $SYMBOL (if resolved) + short mint + copy + favorite */}
      <div className="flex items-center justify-between gap-2 px-2.5 py-1.5 rounded-lg bg-white dark:bg-zinc-950/60 border border-zinc-200 dark:border-zinc-800/60 text-[11px] font-mono">
      ...
      <div className="grid grid-cols-3 gap-2 text-[11px] font-mono">
        <div className="px-2 py-1.5 rounded-lg bg-white dark:bg-zinc-950/50 border border-zinc-200 dark:border-zinc-800/50 space-y-0.5">
      ...
      {/* Actions */}
      <div className="flex items-center gap-2 pt-1 border-t border-zinc-200 dark:border-zinc-800/50">
        {BATON_MINT && (
          <button
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="px-2.5 py-1.5 rounded-lg bg-zinc-800 hover:bg-zinc-700 border border-zinc-300 dark:border-zinc-700 text-zinc-500 dark:text-zinc-400 hover:text-zinc-200 text-[10px] font-mono font-bold transition-colors"
        >
          DEX
        </a>
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="p-1.5 rounded-lg bg-zinc-800 hover:bg-zinc-700 border border-zinc-300 dark:border-zinc-700 text-lime-500 hover:text-lime-300 transition-colors"
        >
